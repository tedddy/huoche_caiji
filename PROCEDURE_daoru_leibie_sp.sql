...
/* CALL daoru_leibie_sp('[标签:类别ID]', '[标签:类别名字]', '[标签:祖父类ID]', '[标签:父类ID]') */

CREATE DEFINER=`root`@`%` PROCEDURE `daoru_leibie_sp`(IN jd_leibie_id_hc VARCHAR(255), jd_leibie_hc VARCHAR(255), jd_leibie_id0_hc VARCHAR(255), jd_leibie_id1_hc VARCHAR(255))
BEGIN
...
ON DUPLICATE KEY UPDATE 
	`jd_leibie_id1` = IF(jd_leibie_id1 IS NULL, `jd_leibie_id1`, jd_leibie_id0_hc),
