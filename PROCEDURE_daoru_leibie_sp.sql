...
/* CALL daoru_leibie_sp('[标签:类别ID]', '[标签:类别名字]', '[标签:祖父类ID]', '[标签:父类ID]', '[标签:类别网址]') */

CREATE DEFINER=`root`@`%` PROCEDURE `daoru_leibie_sp`(IN jd_leibie_id_hc VARCHAR(255), jd_leibie_hc VARCHAR(255), jd_leibie_id0_hc VARCHAR(255), jd_leibie_id1_hc VARCHAR(255), jd_leibie_wangzhi_hc VARCHAR(255))
BEGIN
...
SET jd_leibie_wangzhi_hc = IF(jd_leibie_wangzhi_hc = '' OR jd_leibie_wangzhi_hc LIKE '%???%' OR jd_leibie_wangzhi_hc LIKE '%db:%', NULL, jd_leibie_wangzhi_hc);

INSERT INTO `leibie` (`jd_leibie_id`, `jd_leibie_id0`, `jd_leibie_id1`, `jd_leibie`) VALUES (jd_leibie_id_hc, jd_leibie_id0_hc, jd_leibie_id1_hc, jd_leibie_hc) 
ON DUPLICATE KEY UPDATE 
	...
	`jd_leibie_wangzhi` = IF(jd_leibie_wangzhi_hc IS NULL, `jd_leibie_wangzhi`, jd_leibie_wangzhi_hc);
END